...
- [ ] `src/ragmaker/tools/ask_dir.py` の `_ask_directory_windows` において、`multiple=True` 時は `FOS_PICKFOLDERS` フラグを使用しないよう修正されている。
- [ ] 代わりに `FOS_ALLOWMULTISELECT | FOS_FORCEFILESYSTEM` 等のフラグを使用し、ダイアログ上で複数の項目を選択可能にしている。
- [ ] `GetResults()` から取得したパスのリストに対し、`os.path.isdir()` でフィルタリングを行い、ディレクトリのみを抽出して返すロジックが実装されている。
- [ ] 単一選択（`multiple=False`）時は、従来通り `FOS_PICKFOLDERS` を使用して「フォルダ選択モード」を維持する。
...
## テスト方法
実装担当者は、完了報告前に以下の項目をすべて確認し、チェックを入れ、この文書を更新してcommitすること。
- [ ] Windows 環境で `ragmaker-ask-dir --multiple` を実行し、ファイル選択ダイアログが開くことを確認する。
- [ ] 複数のフォルダを選択し（ファイルも混在させてみる）、出力結果にフォルダのパスのみが含まれていることを確認する。
- [ ] `pytest` を実行し、全テストのパスを確認する。
