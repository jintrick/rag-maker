...
- [x] `src/ragmaker/tools/github_fetch.py` のバグが修正され、単一ファイル指定時に不要なディレクトリが作成されない。
- [x] `src/ragmaker/tools/entry_discovery.py` の `main` 関数で `IOError` が個別にトラップされ、`io_utils.handle_io_error` が呼ばれている。
- [x] `discovery.json` の各ツール定義（`entry_discovery`, `github_fetch`）の `required` 設定が、実装側の `argparse` と完全に一致している。
- [x] `pytest` を実行し、新規・既存の全テストがパスしている。
...
## テスト方法
実装担当者は、完了報告前に以下の項目をすべて確認し、チェックを入れ、この文書を更新してcommitすること。
- [x] `pytest test/test_github_fetch.py` で単一ファイル取得およびディレクトリ取得の両方が正常に動作することを確認した。
- [x] `pytest test/test_entry_discovery.py` を実行し、アサーションの精度（`stderr` への出力確認など）が向上していることを確認した。
- [x] 意図的に `IOError` (書き込み権限のない場所への保存など) を発生させ、適切な JSON エラーが出力されることを確認した。

## 実装状況
実装担当者は、完了報告前に以下の項目をすべて確認し、チェックを入れ、この文書を更新してcommitすること。
- [x] v0.6.1.3 起草
- [x] `github_fetch.py` のクローン・コピーロジック修正完了
- [x] `entry_discovery.py` のエラーハンドリング精密化完了
- [x] `discovery.json` と実装の引数定義同期完了
- [x] 全テストのパスおよび最終確認完了
