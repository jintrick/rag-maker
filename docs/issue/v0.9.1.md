...
- [ ] `browser_open` が永続的なブラウザプロセスを起動（デタッチ）し、CDP接続情報を `.tmp/browser_cdp.json` に保存する。
- [ ] `browser_navigate` および `browser_extract` が `.tmp/browser_cdp.json` を読み込み、既存のブラウザインスタンスに接続して動作する。
- [ ] `browser_extract` からカタログ更新機能を削除し、抽出結果の返却のみに特化させる。
- [ ] `src/ragmaker/utils.py` に `LockedJsonWriter` (Windows/Unix両対応のファイルロック) を実装し、カタログ更新時の排他制御を保証する。
- [ ] `discovery.json` の引数名定義と `rag.md` の指示を完全に同期させる（特に `enrich_discovery` の `--catalog-path`）。
- [ ] `entry_discovery` などのカタログ操作ツールが `LockedJsonWriter` を使用するように修正されている。
