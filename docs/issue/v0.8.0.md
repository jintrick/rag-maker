...
- [ ] `src/ragmaker/tools/http_fetch.py` に SPA 検出およびフォールバック推奨フラグ（メタデータへのステータス付与）が実装されている。
- [ ] `src/ragmaker/tools/browser_fetch.py` が新規作成され、Playwright によるレンダリング後の情報取得が可能になっている。
- [ ] `browser_fetch` のコマンドライン引数（`--url`, `--base-url`, `--output-dir`, `--recursive`, `--depth`）および出力（`catalog.json`）が `http_fetch` と完全な互換性を持っている。
- [ ] `.gemini/commands/rag.md` のワークフローが更新され、`http_fetch` の結果に応じた `browser_fetch` への条件付きフォールバックが定義されている。
- [ ] `discovery.json` に `browser_fetch` の定義が追加されている。
- [ ] `pyproject.toml` に `ragmaker-browser-fetch` スクリプトが登録されている（`playwright` 依存関係は既に存在することを確認済み）。
- [ ] 新規作成したテスト `test/test_browser_fetch.py` および既存のテストがすべてパスしている。
...
## 実装状況
実装担当者は、完了報告前に以下の項目をすべて確認し、チェックを入れ、この文書を更新してcommitすること。
- [ ] `http_fetch.py` のフォールバック判定実装完了
- [ ] `browser_fetch.py` の実装完了 (Playwright/asyncio)
- [ ] `rag.md` のワークフロー更新完了
- [ ] `discovery.json` への登録完了
- [ ] `pyproject.toml` へのスクリプト登録完了
- [ ] 全テストのパス確認
