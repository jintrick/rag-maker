...
- [ ] `src/ragmaker/tools/github_fetch.py` において、フルクローン失敗時に試行の経緯を包含した `RuntimeError` が送出されている。
- [ ] `pytest test/test_github_fetch.py` を実行し、既存の全テストケースがパスしている。
...
## テスト方法
実装担当者は、完了報告前に以下の項目をすべて確認し、チェックを入れ、この文書を更新してcommitすること。
- [ ] `pytest test/test_github_fetch.py` を実行し、正常なクローン動作に影響がないことを確認した。
- [ ] 存在しないリポジトリURLを指定する等の方法で、意図的にクローンを失敗させ、期待通りのエラーメッセージと例外チェーンが出力されることを確認した。
