...
- [x] `ragmaker-install-kb` に `--project-root` 引数が追加され、正常に動作する。
- [x] インストール後の `catalog.json` 内の `path` フィールドが、指定されたプロジェクトルートからの相対パスで記述されている。
- [x] `discovery.json` の `install_kb` 定義が更新され、`project-root` パラメータが含まれている。
...
## テスト方法
実装担当者は、完了報告前に以下の項目をすべて確認し、チェックを入れ、この文書を更新してcommitすること。
- [x] `test/test_install_kb.py` を更新し、`--project-root` を指定した際のカタログ内パスの妥当性を検証するテストケースを追加した。
- [x] `pytest test/test_install_kb.py` を実行し、パスすることを確認した。
- [x] 手動確認: `ragmaker-install-kb --source <SRC> --target-kb-root ./subdir/kb --project-root .` を実行し、カタログ内のパスが `subdir/kb/cache/...` となっていることを確認した。
