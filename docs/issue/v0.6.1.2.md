...
- [x] `src/ragmaker/tools/entry_discovery.py` の `create_initial_catalog` 関数から冗長な `try-except` ブロックが削除され、例外処理が `main` 関数に集約されている。
- [x] `src/ragmaker/tools/entry_discovery.py` が `ragmaker.io_utils.print_json_stdout` を使用して結果を出力している。
- [x] `pytest test/test_entry_discovery.py` を実行し、既存の全テストケースがパスしている。
...
## テスト方法
実装担当者は、完了報告前に以下の項目をすべて確認し、チェックを入れ、この文書を更新してcommitすること。
- [x] `pytest test/test_entry_discovery.py` を実行し、正常系（catalog.jsonの新規作成・更新）および異常系（引数不足）が正しく動作することを確認した。
- [x] 意図的にエラーを発生させ、stderr への出力が重複していないことを確認した。
