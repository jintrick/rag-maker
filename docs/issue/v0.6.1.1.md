...
- [x] `test/test_install_kb.py` ファイルが新規作成され、リポジトリ内に存在している。
- [x] `src/ragmaker/tools/github_fetch.py` が復元されているか、あるいは `pyproject.toml` から `ragmaker-github-fetch` の定義が削除され、不整合が解消されている。
- [x] `src/` 配下の全ファイルから、ドキュメントカタログを指す「discovery.json」の記述が完全に排除されている（マスターツールカタログを指す場合を除く）。
- [x] `src/ragmaker/utils.py` の `print_discovery_data` が `print_catalog_data` に改称され、全ての呼び出し元が修正されている。
- [x] 各ツールのログメッセージおよび docstring が `catalog` または `データ目録` という表現に統一されている。
- [x] `src/ragmaker/data/ask.toml` が `catalog.json` を参照するように修正されている。
- [x] `pytest` を実行し、新規作成した `test/test_install_kb.py` を含む全てのテストがパスしている。
...
## テスト方法
実装担当者は、完了報告前に以下の項目をすべて確認し、チェックを入れ、この文書を更新してcommitすること。
- [x] `test/test_install_kb.py` を新規に作成し、正常系（discovery.json から catalog.json への自動移行）および異常系（ソース欠損等）のテストケースを記述した。
- [x] `pytest test/test_install_kb.py` を単体で実行し、パスすることを確認した。
- [x] `pytest` をプロジェクト全域で実行し、既存機能にデグレがないことを確認した。
- [x] `grep -r "discovery.json" src/` を実行し、マスターツールカタログ以外の文脈でこの文字列が残っていないことを目視で最終確認した。
