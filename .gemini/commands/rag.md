...
### 2. ドキュメントのエンリッチと登録
1. **ドキュメントの収集とエンリッチ情報の生成:**
    - `<プロジェクトルート>/.tmp/cache/` ディレクトリ内のすべてのマークダウンファイル（`.md`）を**直接スキャン**します。
    - 各ファイルを一つずつ**直接読み込み**、それぞれの`title`と`summary`を**あなたの推論能力で生成**します。
    - また、ファイルの内容から元のURLを推測できる場合は `url` も含めてください（`browser_extract` の出力結果を覚えている場合はそれを使用しても良いですが、ファイルスキャンが確実です）。
    - **注意:** `.tmp/` ディレクトリ内のファイルは、ツールを使わずに直接読み書きできます。
2.  **エンリッチ情報の構築**
    -   結果を、以下の形式を持つオブジェクトの**配列**としてメモリ上に構築します。
        ```json
        [
          {"path": "path/to/doc1.md", "url": "...", "title": "...", "summary": "..."},
          {"path": "path/to/doc2.md", "url": "...", "title": "...", "summary": "..."}
        ]
        ```
3.  **一括登録・更新の実行:**
    -   `enrich_discovery` ツールを呼び出します。
    -   `--catalog-path` 引数に、更新対象である `<プロジェクトルート>/.tmp/cache/catalog.json` のパスを指定します。
...
2.  **メタデータの書き込み:**
    ...
    -   `--catalog-path` に `<プロジェクトルート>/.tmp/cache/catalog.json` を指定します。
